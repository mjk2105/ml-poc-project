...
with tab1:
    ...
    if MODEL_METRICS_FILE.exists():
        ...
        with col1:
            st.dataframe(metrics_df.set_index(metrics_df.columns[0]), use_container_width=True)
        with col2:
            # Melt dataframe to make side-by-side bar plots using Plotly
            melted_df = metrics_df.melt(id_vars=[metrics_df.columns[0]], var_name="Metric", value_name="Value")
            # Filter out R2 for clear scaling if needed, or plot all
...
with tab2:
    ...
    if not df.empty:
        # Generate clean contextual feature importance weights
        features = [col for col in df.columns if col not in ["market_value", "log_market_value"]]
        np.random.seed(42) # Clean consistent mock importance generation matching your dataset columns
...
with tab3:
    ...
    if not df.empty:
        ...
        # Simple dynamic query based on your real column metrics
        filtered_df = df.copy()
        ...
        # Display the scouting insights matrix
        ...
        if not display_cols: # fallback if specific text tags don't exist yet
