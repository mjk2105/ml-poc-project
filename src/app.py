...
Run from the project root or notebooks folder:
...
<style>
  ...
  .kpi-card {
    ...
    padding:16px 18px; text-align:center; margin:5px 0;
  }
  .kpi-label { font-size:11px; color:#8891a7; text-transform:uppercase; letter-spacing:1px; }
  .kpi-value { font-size:22px; font-weight:700; color:#fafafa; margin:4px 0; }
  .kpi-sub   { font-size:11px; color:#27ae60; }
  .player-card {
    background:linear-gradient(135deg,#1e2235,#252a3d);
    border:1px solid #2e3450; border-radius:14px;
    padding:18px; margin:6px 0;
  }
  .pname { font-size:16px; font-weight:700; color:#fafafa; }
  .psub  { font-size:12px; color:#8891a7; margin-bottom:8px; }
  .tag { display:inline-block; padding:3px 10px; border-radius:20px;
         font-size:11px; font-weight:600; margin:2px; }
  .tag-buy   { background:#1a3d2b; color:#27ae60; border:1px solid #27ae60; }
  .tag-risk  { background:#3d1a1a; color:#e74c3c; border:1px solid #e74c3c; }
  .tag-prime { background:#1a2e3d; color:#3498db; border:1px solid #3498db; }
  .tag-left  { background:#2e1a3d; color:#9b59b6; border:1px solid #9b59b6; }
  .section-hd {
    font-size:20px; font-weight:700; color:#fafafa;
    border-left:4px solid #27ae60; padding-left:12px;
    margin:22px 0 14px 0;
  }
  .subtitle  { font-size:14px; color:#8891a7; margin-bottom:18px; }
  .gap-pill  {
    display:inline-block; background:linear-gradient(90deg,#1a3d2b,#1e4d35);
    color:#27ae60; border:1px solid #27ae60; border-radius:20px;
...
def load_data():
    # Engineered outfield dataset — D02 output, fillna(0) matches D03
    df = pd.read_csv(DATA / "df_engineered_outfield.csv").fillna(0)

    # Player names from profiles (not in engineered CSV)
    try:
...
def load_model():
    ...
    if not path.exists():
        return None
    m = XGBRegressor()
    m.load_model(str(path))   # D03 used xgb_model.save_model() — JSON format
    return m
...
def radar_fig(t, c):
    ...
    fig.update_layout(
        polar=dict(radialaxis=dict(visible=True, range=[0,1],
                                   tickfont=dict(color="#8891a7", size=8))),
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color="#fafafa", size=11), showlegend=True,
        legend=dict(orientation="h", y=-0.15),
        margin=dict(t=20,b=40,l=20,r=20), height=270,
    )
    return fig


def gauge_fig(score: float):
    col = "#27ae60" if score < 0.25 else "#f39c12" if score < 0.55 else "#e74c3c"
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=round(score * 100),
        number={"suffix":"%","font":{"color":"#fafafa","size":26}},
        gauge={
            "axis":  {"range":[0,100],"tickcolor":"#8891a7"},
            "bar":   {"color":col,"thickness":0.25},
            "bgcolor":"#1e2235",
            "steps": [{"range":[0,25],"color":"#1a3d2b"},
                      {"range":[25,55],"color":"#3d2e1a"},
                      {"range":[55,100],"color":"#3d1a1a"}],
        },
        title={"text":"Injury Risk","font":{"color":"#fafafa","size":12}},
    ))
...
with st.sidebar:
    ...
    st.markdown("<div style='color:#8891a7;font-size:13px;margin-bottom:16px;'>"
                "Value-for-Money · Group 7</div>", unsafe_allow_html=True)
    st.markdown("<hr style='border-color:#2e3450;'>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style='font-size:11px;color:#8891a7;line-height:2;'>
    <b style='color:#fafafa;'>Dataset</b><br>
    {n_total:,} outfield players<br>
    <span style='color:#27ae60;'>{n_under:,} undervalued</span><br><br>
    <b style='color:#fafafa;'>Model</b><br>XGBoost (D03)<br>
    RMSLE · MAE · R²
...
if page == "🏠  Overview":
    ...
    with col_l:
        ...
        <b style='color:#27ae60;'>€15M gap is your scouting opportunity</b>.
        </p>""", unsafe_allow_html=True)

        st.markdown("<div class='section-hd'>Three Tools in One App</div>",
                    unsafe_allow_html=True)
        for icon, title, desc in [
            ("🔍", "Budget Clone Finder",
             "Select any world-class player — get 3 statistically similar alternatives "
             "at a fraction of the cost."),
            ("💼", "Scout Search",
             "Set your budget and target position — get the top undervalued gems on the market."),
            ("📊", "Model Story",
             "Understand which player attributes drive value and how the model was built."),
        ]:
            st.markdown(f"""
            <div class='player-card' style='padding:12px 16px;'>
              <span style='font-size:20px;'>{icon}</span>
              <b style='font-size:14px;margin-left:8px;'>{title}</b>
              <div style='color:#8891a7;font-size:12px;margin-top:4px;'>{desc}</div>
            </div>""", unsafe_allow_html=True)

    with col_r:
        st.markdown("<div class='section-hd'>Live Dataset Stats</div>",
                    unsafe_allow_html=True)
        ...
        fi_dict    = dict(zip(
            [f for f in FEATURES_TREE if f in df.columns],
    ...
    st.markdown("<hr class='divider'>", unsafe_allow_html=True)
    st.markdown("<div class='section-hd'>Top 10 Biggest Valuation Gaps — Live</div>",
                unsafe_allow_html=True)
    ...
    fig_t10.add_trace(go.Bar(
        ...
        orientation="h", marker_color="#27ae60", marker_line_width=0, opacity=0.85,
    ))
    fig_t10.update_layout(
        barmode="overlay", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color="#fafafa"), legend=dict(orientation="h", y=1.08),
        xaxis=dict(title="€ Millions", gridcolor="#2e3450"),
        yaxis=dict(gridcolor="#2e3450", autorange="reversed"),
        height=380, margin=dict(t=40, b=20, l=10, r=20),
        title="Green extending past red = buy signal",
    )
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with c2:
        ...
        chosen = st.selectbox(
            f"Select a {pos_sel} player (top 60 by market value)",
            pool["player_name"].tolist(),
        )
    ...
    st.markdown("<div class='section-hd'>3 Budget Alternatives</div>",
                unsafe_allow_html=True)
    ...
    for rank, (_, c) in enumerate(clones.iterrows()):
        ...
        rl_c, _ = risk_label(c["injury_risk_score"])
        prime   = 21 <= c["age"] <= 27

        with st.expander(
            f"#{rank+1}  {c['player_name']}  ·  {c['position_clean']}  "
            f"·  €{c['market_value_eur']/1e6:.1f}M  ·  {sim:.0f}% similarity",
            expanded=(rank == 0),
        ):
            ...
            with left:
                ...
                tags += '<span class="tag tag-risk">⚠ High Risk</span>' \
                        if c["injury_risk_score"] > 0.35 else ""
                tags += '<span class="tag tag-left">🦶 Left Foot</span>' \
                        if c["is_left_footed"] else ""
                ...
                <div class='player-card'>
                  ...
                  <table style='width:100%;font-size:13px;border-collapse:collapse;'>
                    <tr><td style='color:#8891a7;padding:3px 0;'>Market Price</td>
                        <td style='text-align:right;'><b>€{c['market_value_eur']/1e6:.1f}M</b></td></tr>
                    <tr><td style='color:#8891a7;padding:3px 0;'>Predicted Value</td>
                        <td style='text-align:right;'><b style='color:#27ae60;'>€{c['predicted_value']/1e6:.1f}M</b></td></tr>
                    <tr><td style='color:#8891a7;padding:3px 0;'>Valuation Gap</td>
                        <td style='text-align:right;'><span class='gap-pill'>+€{gap_m:.1f}M</span></td></tr>
                    <tr><td style='color:#8891a7;padding:3px 0;'>Savings vs Target</td>
                        <td style='text-align:right;'><b style='color:#3498db;'>€{savings:.1f}M</b></td></tr>
                    <tr><td style='color:#8891a7;padding:3px 0;'>G+A per 90</td>
                        <td style='text-align:right;'><b>{c['goal_contributions_per_90']:.2f}</b></td></tr>
                    <tr><td style='color:#8891a7;padding:3px 0;'>Min / Game</td>
                        <td style='text-align:right;'><b>{c['minutes_per_game_proxy']:.0f}'</b></td></tr>
                    <tr><td style='color:#8891a7;padding:3px 0;'>Days Injured</td>
                        <td style='text-align:right;'><b>{c['days_missed']:.0f} days</b></td></tr>
                    <tr><td style='color:#8891a7;padding:3px 0;'>Similarity</td>
                        <td style='text-align:right;'><b>{sim:.0f}%</b></td></tr>
                  </table>
                </div>""", unsafe_allow_html=True)

            with mid:
                st.plotly_chart(radar_fig(t, c), use_container_width=True,
                                key=f"radar_{rank}")

            with right:
                st.plotly_chart(gauge_fig(c["injury_risk_score"]),
                                use_container_width=True, key=f"gauge_{rank}")
                st.markdown(f"""
                <div style='text-align:center;margin-top:6px;'>
                  <div style='color:#8891a7;font-size:11px;'>Days Injured</div>
                  <div style='font-size:22px;font-weight:700;'>{c['days_missed']:.0f}</div>
                  <div style='color:#8891a7;font-size:11px;margin-top:4px;'>Risk Score</div>
                  <div style='font-size:22px;font-weight:700;'>{c['injury_risk_score']:.2f}</div>
                </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<div class='subtitle'>Set your budget and target position. "
                "The model returns the most undervalued players on the market.</div>",
                unsafe_allow_html=True)
    st.markdown("<hr class='divider'>", unsafe_allow_html=True)
    ...
    if results.empty:
        st.warning("No undervalued players match your filters. "
                   "Try increasing budget or relaxing injury risk.")
        st.stop()

    st.markdown(
        f"<div class='section-hd'>{len(results)} Result(s) — "
        f"{pos_sel} · Budget ≤ €{budget_m}M</div>",
        unsafe_allow_html=True,
    )
    ...
    fig_s.add_trace(go.Bar(
        name="Asking Price (€M)", y=results["player_name"].tolist(),
        x=(results["market_value_eur"] / 1e6).tolist(),
        orientation="h", marker_color="#e74c3c", marker_line_width=0,
    ))
    fig_s.add_trace(go.Bar(
        name="Predicted Value (€M)", y=results["player_name"].tolist(),
        x=(results["predicted_value"] / 1e6).tolist(),
        orientation="h", marker_color="#27ae60", marker_line_width=0, opacity=0.8,
    ))
    fig_s.update_layout(
        barmode="overlay", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font=dict(color="#fafafa"), legend=dict(orientation="h", y=1.1),
        xaxis=dict(title="€ Millions", gridcolor="#2e3450"),
        yaxis=dict(gridcolor="#2e3450", autorange="reversed"),
        height=max(240, len(results) * 46),
    ...
    for rank, (_, r) in enumerate(results.iterrows()):
        ...
        c0, c1, c2, c3, c4, c5, c6 = st.columns([0.4, 2.5, 1, 1.2, 1, 1, 1])
        with c0:
            st.markdown(
                f"<div style='font-size:26px;text-align:center;padding-top:14px;'>"
                f"{medals[rank]}</div>", unsafe_allow_html=True)
        with c1:
            tags  = '<span class="tag tag-buy">💰 BUY</span>'
            tags += '<span class="tag tag-prime">🌟 Prime</span>' if prime else ""
            tags += '<span class="tag tag-risk">⚠ Risk</span>' \
                    if r["injury_risk_score"] > 0.35 else ""
        ...
        with c2:
            st.metric("Price",    f"€{r['market_value_eur']/1e6:.1f}M")
        with c3:
            st.metric("Predicted",f"€{r['predicted_value']/1e6:.1f}M",
                      delta=f"+€{gap_m:.1f}M")
        with c4:
        ...
        st.markdown("<hr class='divider' style='margin:4px 0;'>",
                    unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("<div class='subtitle'>How the XGBoost model was built, what it learned, "
                "and how confident we are in its predictions.</div>",
                unsafe_allow_html=True)
    st.markdown("<hr class='divider'>", unsafe_allow_html=True)

    # Model comparison cards
    st.markdown("<div class='section-hd'>Model Comparison — D03 Results</div>",
                unsafe_allow_html=True)
    for col, (name, color, role, rmsle, mae, r2) in zip(st.columns(3), [
        ...
        ("XGBoost",          "#27ae60", "⭐ Primary",  0.81, 0.95, 0.74),
    ]):
        with col:
            ...
            <div class='player-card' style='{border}'>
              ...
              <table style='width:100%;font-size:13px;border-collapse:collapse;'>
                <tr><td style='color:#8891a7;padding:4px 0;'>RMSLE ↓</td>
                    <td style='text-align:right;'><b>{rmsle}</b></td></tr>
                <tr><td style='color:#8891a7;padding:4px 0;'>MAE (€M) ↓</td>
                    <td style='text-align:right;'><b>€{mae}M</b></td></tr>
                <tr><td style='color:#8891a7;padding:4px 0;'>R² ↑</td>
                    <td style='text-align:right;'><b>{r2}</b></td></tr>
              </table>
            </div>""", unsafe_allow_html=True)
    ...
    with left_col:
        st.markdown("<div class='section-hd'>Feature Importance (Live from model)</div>",
                    unsafe_allow_html=True)
        feats      = [f for f in FEATURES_TREE if f in df.columns]
        importances = pd.Series(
            xgb.feature_importances_[:len(feats)],
            index=[f.replace("_", " ") for f in feats],
        ).sort_values(ascending=True)

        fi_colors = [
            "#27ae60" if v >= importances.quantile(0.66)
        ...
        fig_fi.update_layout(
            ...
            xaxis=dict(title="Feature importance (gain)", gridcolor="#2e3450"),
            yaxis=dict(gridcolor="#2e3450"),
            height=420, margin=dict(t=10, b=20, l=10, r=20),
        )
        st.plotly_chart(fig_fi, use_container_width=True)

    with right_col:
        st.markdown("<div class='section-hd'>Market Value Distribution by Position</div>",
                    unsafe_allow_html=True)
        ...
        for pos in ["DEF", "MID", "WNG", "ATT"]:
            ...
            fig_viol.add_trace(go.Violin(
                y=vals, name=pos, box_visible=True, line_color=POS_COLORS[pos],
                fillcolor=POS_COLORS[pos], opacity=0.5, x0=pos,
            ))
        fig_viol.update_layout(
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#fafafa"), showlegend=False,
            yaxis=dict(title="Market Value (€M, capped 95th pct)", gridcolor="#2e3450"),
    ...
    st.markdown("<hr class='divider'>", unsafe_allow_html=True)
    st.markdown("<div class='section-hd'>ML Pipeline (D01 → D05)</div>",
                unsafe_allow_html=True)
    p_cols = st.columns(5)
    for col, (code, color, title, desc) in zip(p_cols, [
        ("D01","#3498db","Data Collection",
         "4 Transfermarkt CSVs · 22K+ players · 10 raw features"),
        ("D02","#9b59b6","Feature Engineering",
         "16 features · log1p target · GK split · per-90 rates"),
        ("D03","#e67e22","Model Selection",
         "Ridge → RF → XGBoost · 80/20 split · RMSLE/MAE/R²"),
        ("D04","#1abc9c","Visualisation",
         "3-plot suite: raw data · engineering impact · model comparison"),
        ("D05","#27ae60","Deployment",
         "Streamlit · Clone Finder · Scout Search · Model Story"),
    ]):
        with col:
            st.markdown(f"""
            <div class='player-card' style='border-color:{color};padding:12px;text-align:center;'>
              ...
              <div style='font-size:11px;color:#8891a7;margin-top:6px;
